...
Agent Q's audit on 2026-04-22 (post-merge) flagged the leftover `KeyboardSensor` + `laneKeyboardCoordinateGetter` as demo-dishonest tech debt: live code, tested only indirectly, claiming an accessibility surface it does not deliver. This ADR accompanies the cleanup PR that removes it.
