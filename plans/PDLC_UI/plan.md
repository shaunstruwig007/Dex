...
> **FROZEN 2026-04-24 — pdlc-ui parked.** The live plan is [`plans/skill-pipeline/README.md`](../skill-pipeline/README.md). This document is preserved for reference; its contents do not drive current work. Skills-to-100% is the only goal of the current cycle; if the pdlc-ui phase resumes, update or supersede this banner.
...
9. **~~Gatekeeper (`/gatekeeper-custom`)~~ — removed 2026-04-24.** The dedicated PR-merge Dex skill for **`pdlc-ui/`** was **deleted from the vault** when `pdlc-ui` was parked in favour of the [skill pipeline](../skill-pipeline/README.md). **If `pdlc-ui` PRs resume:** use Cursor **`babysit`** (`.cursor/skills-cursor/babysit/SKILL.md`) plus the R16 checklist in frozen [engineering-guardrails.md](./engineering-guardrails.md) and [skill-agent-map § Engineering](./skill-agent-map.md#engineering--merge-gate-pdlc-ui-repo). **Recover the last committed PR-gate SKILL** (this branch never had `gatekeeper-custom/` in git — it was a short-lived local rename): `git show freeze/skills-pipeline-pivot^:.claude/skills/moneypenny-custom/SKILL.md` (MoneyPenny PR gate, pre–persona remap). Requires **`gh auth login`** once for CI loops.
...
| 2026-04-22 | **S3A.1 cleanup — docs & DnD hygiene** (pre-S3A.2) | [PR TBD `chore/s3a1-cleanup-docs-and-dnd-hygiene`]; [pdlc-ui/src/components/ideas/board-dnd.tsx](../../pdlc-ui/src/components/ideas/board-dnd.tsx); [pdlc-ui/src/components/ideas/swim-lane.tsx](../../pdlc-ui/src/components/ideas/swim-lane.tsx); [pdlc-ui/src/schema/required-fields-parity.test.ts](../../pdlc-ui/src/schema/required-fields-parity.test.ts); [pdlc-ui/docs/adr/0003-dnd-shape-and-html5-ban.md](../../pdlc-ui/docs/adr/0003-dnd-shape-and-html5-ban.md); [pdlc-ui/docs/design-log/2026-04-22-pivot-to-modal.md](../../pdlc-ui/docs/design-log/2026-04-22-pivot-to-modal.md); [pdlc-ui/docs/design/board-layout.md](../../pdlc-ui/docs/design/board-layout.md); [sprint-backlog.md § S3A.2 + § S3A.3](./sprint-backlog.md); [seeds/_superseded/s3a3-discovery-automation.md](./seeds/_superseded/s3a3-discovery-automation.md) | **Agent Q post-S3A.1 audit** (GAP × 1, SOFT × 5) paid down in one PR before S3A.2 enters Plan mode. **GAP — A11y demo-honesty:** removed dead `KeyboardSensor` + `laneKeyboardCoordinateGetter` from `board-dnd.tsx` (shipped but clamped to source lane by default modifiers; `Space → Arrow → Space` never crossed) — keyboard cross-lane is now explicitly the `Actions → Move to…` submenu until an S3A.3 revival with `DragOverlay` or a custom modifier. **SOFT — schema parity:** new Vitest at `pdlc-ui/src/schema/required-fields-parity.test.ts` locks `REQUIRED_BRIEF_FIELDS` (`src/content/brief-steps.ts`) against `REQUIRED_FOR_COMPLETE_BRIEF` (`src/schema/initiative.ts`) so the two runtime sources of truth can't silently drift. **SOFT — dead code:** `onListDragOver` / `onListDrop` removed from `swim-lane.tsx` (already-dead since S3A.1 pass-2); stale HTML5-drag + `KeyboardSensor` comments in `initiative-card.tsx` updated. **SOFT — contract-doc drift (modal pivot):** `board-layout.md` §5 rewritten from "non-modal side panel" to "Initiative Modal — URL-addressable, tabbed, ~70vw × ~85vh" (cross-links the new S3A.2 seed); §6 focused-column deferred to S3A.3; §7 L155 "Space + arrows + Enter + Esc" DnD claim dropped. Old side-panel reasoning preserved in a new **design-log** entry ([design-log/2026-04-22-pivot-to-modal.md](../../pdlc-ui/docs/design-log/2026-04-22-pivot-to-modal.md)). `sprint-backlog.md` §S3A.2 replaced with the modal scope; **new §S3A.3 block** carries the old automation surface (prefill, kickoff, jobs table, focused-column, `KeyboardSensor` revival). Old S3A.2 seed moved to `seeds/_superseded/s3a3-discovery-automation.md`. `plan.md` + `s3a1-brief-wizard-interactions.md` + `s3b-discovery-research.md` + `schema-initiative-v0.md` cross-refs swapped "side panel" → "Initiative Modal". **SOFT — missing ADR:** new **ADR-0003 — DnD shape + HTML5 ban** documents `@dnd-kit/core`-only, HTML5 `draggable` banned on card `<li>`s (e2e regression guard in `e2e/dnd.spec.ts`), `Actions → Move to…` as the shipped keyboard path, `KeyboardSensor` deferred until `translate3d` bounds are widened. **SOFT — base-ui vs Radix slip:** S3A.2 seed L31 corrected from "Radix `Dialog`" to "Base-UI `Dialog` via the shadcn wrapper" (`pdlc-ui/src/components/ui/dialog.tsx` wraps `@base-ui/react/dialog`). **Zero contract change, zero behaviour change.** CI: typecheck + lint + Vitest 72/72 (including the new parity test) + Playwright 20/20 all green. Predicts Q re-run clean READY FOR BUILD for S3A.2. |
| 2026-04-21 | **Brief shrink (CPO pass) + S3B committed + ICP placeholder** | [.claude/skills/pdlc-brief-custom/SKILL.md](../../.claude/skills/pdlc-brief-custom/SKILL.md); [schema-initiative-v0 §4.2 + §4.3 + §6 + §8](./schema-initiative-v0.md); [seeds/_archived-2026-04-24/s3a1-brief-wizard-interactions.md Deliverable 0](./seeds/_archived-2026-04-24/s3a1-brief-wizard-interactions.md); [seeds/_superseded/s3a3-discovery-automation.md Prefill + Provider swap](./seeds/_superseded/s3a3-discovery-automation.md); [seeds/_archived-2026-04-24/s3b-discovery-research.md](./seeds/_archived-2026-04-24/s3b-discovery-research.md); [sprint-backlog — S3A.1 + S3A.2 + S3B blocks](./sprint-backlog.md); `System/icp.md` placeholder (Shaun authors 2026-04-22) | **CPO decision:** the brief is a **thesis gate**, not a PRD. Shrunk to **three questions** — *why* (`coreValue`), *who* (`targetUsers`), *what* (`problem`). Scope / assumptions / success metrics move to **discovery** (S3B) and **spec** (`/agent-prd`). `REQUIRED_BRIEF_FIELDS` narrows; legacy `scopeIn` / `scopeOut` / `assumptions` / `constraints` / `successDefinition` stay **optional** in `briefSchema` for backward compat (no data migration). Lands in **S3A.1 Deliverable 0** (reverses the earlier "defer question copy" call — it's a wizard-UX change). **S3A.2 prefill** expands scope from 2 → 3 fields to match. **S3B committed** as `/pdlc-discovery-research-custom`: replaces the S3A.2 kickoff stub behind the same `DiscoveryResearchProvider` interface (no route / job / UI change); runs on kickoff + manual re-run + **weekly sweep across all `discovery`-column cards** to accumulate customer context over time; composes existing Dex intel (`/customer-intel`, `/intelligence-scanning`, `/weekly-exec-intel`, `/meeting-prep`) — UI consumes what Dex already does. Seed reserves the slot with a locked interface but defers Q1–Q10 (research taxonomy, source storage, confidence scoring, ICP shape, LLM wiring, staleness diffing, observability, partial-failure, `/agent-prd` boundary, re-run-on-brief-edit UX) to a **2026-04-22+ deep-dive with Shaun + PO** before Build. **ICP placeholder:** `System/icp.md` — to be authored by Shaun 2026-04-22 — is a **blocker for the weekly sweep** (scores strategic fit; filters competitor analysis). Schema §4.2 / §4.3 / §6 / §8 updated same-sweep: brief required set narrowed + legacy-fields backward-compat note; `/pdlc-discovery-research-custom` added to §8 I/O table with its full reads / writes; `skill_run` known-ids list extended with the S3B id. Sprint-backlog Bar table gains an S3B row. No S3 / S3A contract widened. |
| 2026-04-24 | **007 persona re-map** (personal-Dex workflow) | [.claude/skills/moneypenny-custom/SKILL.md](../../.claude/skills/moneypenny-custom/SKILL.md); ~~`.claude/skills/gatekeeper-custom/SKILL.md`~~ *(added then **removed** 2026-04-24 — recover from git parent commit)*; [plans/Research/moneypenny-strategy.md](../Research/moneypenny-strategy.md); [schema-initiative-v0 §3 + §4.2 + §4.3 + §6 + §8](./schema-initiative-v0.md); [skill-agent-map.md](./skill-agent-map.md); [claude-design-orchestration-doc-prompt.md](./claude-design-orchestration-doc-prompt.md); [seeds/_archived-2026-04-24/s3b-discovery-research.md](./seeds/_archived-2026-04-24/s3b-discovery-research.md); [System/discovery-sources.yaml](../../System/discovery-sources.yaml) | **Persona re-map only — zero behaviour / contract change.** Reflects Shaun's departure from Wyzetalk + shift from PR-merge-tracked `pdlc-ui` to personal-Dex chat-driven workflow. Renames: **(a)** `/pdlc-discovery-research-custom` (codename "Bond") → **`/moneypenny-custom`** (Moneypenny — per-initiative intelligence debriefer). **(b)** Old `/moneypenny-custom` (CI/PR gate) → **`/gatekeeper-custom`** — later **deleted** 2026-04-24 (see next Progress row). **(c)** "Bond" codename migrated to deferred **`/bond-prd-custom`** — PRD author, **map-only this pass** (SKILL.md authored later); **supersedes `/agent-prd`** in personal-Dex mode (`/agent-prd` remains the compatibility shim until Bond PRD ships; field-level contract identical). All Mode A / Mode B / cost-ceiling / cadence / schema contracts unchanged. `plans/Research/bond-strategy.md` → `moneypenny-strategy.md`. Schema §6 known-ids registry: retired `pdlc-discovery-research-custom`, added `moneypenny-custom`, reserved `bond-prd-custom`. Older Progress log rows and sprint-backlog references retain the original slug (historical accuracy); all **live** cross-refs in `System/icp.md`, `.claude/skills/pdlc-brief-custom/`, `.claude/skills/felix-custom/`, `plans/Research/felix-strategy.md`, `06-Resources/Research/Industry_reports/README.md` updated to new slug with rename-awareness note. **Out of scope this pass:** authoring `.claude/skills/bond-prd-custom/SKILL.md`, editing `.claude/skills/agent-prd/`, any `pdlc-ui` runtime code (DiscoveryResearchProvider stub left with the historical skill path — swap if the project revives). |
| 2026-04-24 | **Skill pipeline Phase 0 — park `pdlc-ui` + remove Gatekeeper** | [`plans/skill-pipeline/README.md`](../skill-pipeline/README.md); [`plans/skill-pipeline/lessons-from-skills.md`](../skill-pipeline/lessons-from-skills.md); [`.claude/hooks/session-start.sh`](../../.claude/hooks/session-start.sh); FROZEN banners across `plans/PDLC_UI/`; **deleted** `.claude/skills/gatekeeper-custom/` | **`pdlc-ui` plan corpus frozen** — live work moves to the skill pipeline README. **`/gatekeeper-custom`** removed from the vault; PR discipline → Cursor **`babysit`** + frozen R16 tables. **SessionStart** hook appends **`System/icp.md`** + skill-pipeline README excerpt. **Git tag** `freeze/skills-pipeline-pivot` marks this commit. Recover last committed PR-gate SKILL: `git show freeze/skills-pipeline-pivot^:.claude/skills/moneypenny-custom/SKILL.md`. |
