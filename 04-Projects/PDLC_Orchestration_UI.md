...
| 2026-04-22 | **Sprint S3A.1** — Brief wizard + board interaction polish | Branch `feat/s3a1-brief-wizard-interactions`; [PR #3](https://github.com/shaunstruwig007/Dex/pull/3); [pdlc-ui/src/components/ideas/](../pdlc-ui/src/components/ideas/); [pdlc-ui/src/components/brief/brief-wizard-dialog.tsx](../pdlc-ui/src/components/brief/brief-wizard-dialog.tsx); [pdlc-ui/content/pdlc-brief-steps.json](../pdlc-ui/content/pdlc-brief-steps.json); [pdlc-ui/docs/design/board-layout.md](../pdlc-ui/docs/design/board-layout.md); [lifecycle-transitions.md — Cross-lane DnD](../plans/PDLC_UI/lifecycle-transitions.md); [tech-stack.md §3.5](../plans/PDLC_UI/tech-stack.md); [plan-mode-prelude.md](../plans/PDLC_UI/plan-mode-prelude.md) | **Shipped:** Brief **shrunk to 3 required fields** (`problem`, `targetUsers`, `coreValue`) via `briefSchema.superRefine` + `missingForCompleteBrief` helper — the same helper guards `saveBriefAndTransition` server-side (no drift); legacy `scopeIn` / `scopeOut` / `assumptions` / `constraints` / `successDefinition` stay optional (no migration). Wizard gains StepRail with red-dot required indicators, asterisks + "Required" copy, summary composite with click-to-edit jumps, dual save buttons (`Save brief only` + `Save brief & start discovery` — server-identical this sprint; kickoff side-effect deferred to S3A.2 for M1 honesty). **Cross-lane pointer DnD** via `@dnd-kit/core` only (`@dnd-kit/sortable` deliberately **not** installed — Q-alt.1); `PointerSensor` 6px activation; same pure `canTransition` on drag; illegal lanes dim with the existing `humanError` tooltip; `same_lifecycle` drop is silent (Q-P2.1); `idea → discovery` opens the wizard; `→ parked` opens the existing dialog. **Keyboard cross-lane surface** this sprint = the `Actions → Move to…` submenu — pixel-perfect Space/Arrow/Space keyboard DnD deferred to S3A.2 under focused-column mode (M1 honesty). **Board shell** per [board-layout.md §1–§4](../pdlc-ui/docs/design/board-layout.md): chrome-light 48px sticky bar, board becomes scroll container, elastic 7-lane grid, collapsible right-edge **parked rail** (localStorage), compact/comfortable/detailed **density toggle** (CSS vars + localStorage); truncated `problem.value` preview on the card face when `brief.complete`. **Pass-2 correction (`3875475`):** removed native HTML5 `draggable={!isParked}` from the card `<li>` after human validation revealed it preempted dnd-kit's `PointerSensor` on real-browser mousedown — the browser fires `dragstart` before `pointermove`, so dnd-kit never crossed its 6px activation. Playwright's `page.mouse.*` synthesises pointer/mouse events but not HTML5 `drag*`, so the e2e suite had a blind spot. Within-lane pointer reorder (HTML5 in S2) is now an **S3A.2 carry-over** — keyboard `Alt+↑/↓` + menu cover within-lane this sprint. Added an e2e regression guard that fails if the card ever re-advertises `draggable="true"`. **Agents M (CPO) + Q (CTO)** added as plan-mode critique step via [plan-mode-prelude.md](../plans/PDLC_UI/plan-mode-prelude.md) + new `.claude/skills/agent-m-cpo-custom/` + `agent-q-cto-custom/` skill files (correcting earlier confusion with MoneyPenny + QMD search). R16 same-PR co-change shipped across `lifecycle-transitions.md`, `tech-stack.md §3.5`, `schema-initiative-v0 §4.2`, and `.claude/skills/pdlc-brief-custom/SKILL.md`. CI green; Vitest 71/71; Playwright 20/20. MoneyPenny Mode B + post-pass-2 R16 audit both clean (MERGE verdict). **Tech — next sprint (S3A.2) MUST preserve AND deliver:** preserve `briefSchema` + `REQUIRED_FOR_COMPLETE_BRIEF` + `missingForCompleteBrief`, `eventSchema` (incl. `skill_run` payload), atomic `POST /api/initiatives/:id/brief` + `missing_required_fields` 422 contract, `canTransition` + `deriveHasBrief`, cross-lane DnD legality gate + `same_lifecycle` silent no-op, dual save-button surface, `Actions → Move to…` keyboard cross-lane fallback, the `<li>` must-never-advertise-`draggable="true"` regression guard. Deliver: within-lane pointer reorder via dnd-kit (`useDroppable`-per-slot + `neighboursForSwap` + `computeMidpointSortOrder`, no HTML5 `draggable`) + a CDP `Input.dispatchDragEvent` e2e so the pass-2 defect class cannot escape again. |
| 2026-04-22 | **S3A.1 cleanup** — docs & DnD hygiene (pre-S3A.2) | Branch `chore/s3a1-cleanup-docs-and-dnd-hygiene`; [pdlc-ui/src/components/ideas/board-dnd.tsx](../pdlc-ui/src/components/ideas/board-dnd.tsx); [pdlc-ui/src/components/ideas/swim-lane.tsx](../pdlc-ui/src/components/ideas/swim-lane.tsx); [pdlc-ui/src/schema/required-fields-parity.test.ts](../pdlc-ui/src/schema/required-fields-parity.test.ts); [pdlc-ui/docs/adr/0003-dnd-shape-and-html5-ban.md](../pdlc-ui/docs/adr/0003-dnd-shape-and-html5-ban.md); [pdlc-ui/docs/design-log/2026-04-22-pivot-to-modal.md](../pdlc-ui/docs/design-log/2026-04-22-pivot-to-modal.md); [pdlc-ui/docs/design/board-layout.md](../pdlc-ui/docs/design/board-layout.md); [sprint-backlog.md §S3A.2 + §S3A.3](../plans/PDLC_UI/sprint-backlog.md); [seeds/_superseded/s3a3-discovery-automation.md](../plans/PDLC_UI/seeds/_superseded/s3a3-discovery-automation.md) | **Shipped (one maintenance PR, zero contract change, zero behaviour change):** Agent Q's post-S3A.1 audit (GAP × 1, SOFT × 5) paid down before S3A.2 enters Plan mode. **GAP — A11y demo-honesty:** dead `KeyboardSensor` + `laneKeyboardCoordinateGetter` removed from `board-dnd.tsx` (shipped but clamped to source lane by default modifiers; `Space → Arrow → Space` never crossed) — the `Actions → Move to…` submenu is now explicitly the only shipped keyboard cross-lane path until an S3A.3 revival with a `DragOverlay` or custom modifier. **SOFT — schema parity:** new Vitest at `src/schema/required-fields-parity.test.ts` locks `REQUIRED_BRIEF_FIELDS` (`src/content/brief-steps.ts`) against `REQUIRED_FOR_COMPLETE_BRIEF` (`src/schema/initiative.ts`) so the two runtime sources of truth can't silently drift. **SOFT — dead code:** `onListDragOver` / `onListDrop` purged from `swim-lane.tsx` (already-dead since S3A.1 pass-2); stale HTML5-drag + `KeyboardSensor` comments in `initiative-card.tsx` updated. **SOFT — contract-doc drift (modal pivot):** `board-layout.md` §5 rewritten from "non-modal side panel" to "Initiative Modal — URL-addressable, tabbed, ~70vw × ~85vh"; §6 focused-column deferred to S3A.3; §7 L155 "Space + arrows + Enter + Esc" DnD claim dropped. Old side-panel reasoning preserved in a new **design-log** entry ([design-log/2026-04-22-pivot-to-modal.md](../pdlc-ui/docs/design-log/2026-04-22-pivot-to-modal.md)). `sprint-backlog.md` §S3A.2 replaced with the modal scope; **new §S3A.3 block** carries the old automation surface (prefill, kickoff, jobs table, focused-column, `KeyboardSensor` revival). Old S3A.2 seed moved to `seeds/_superseded/s3a3-discovery-automation.md`. Cross-refs in `plan.md` + `s3a1-brief-wizard-interactions.md` + `s3b-discovery-research.md` + `schema-initiative-v0.md` swapped "side panel" → "Initiative Modal". **SOFT — missing ADR:** new **ADR-0003 — DnD shape + HTML5 ban** documents `@dnd-kit/core`-only, HTML5 `draggable` banned on card `<li>`s (e2e regression guard in `e2e/dnd.spec.ts`), `Actions → Move to…` as the shipped keyboard path, `KeyboardSensor` deferred until `translate3d` bounds are widened. **SOFT — base-ui slip:** S3A.2 seed L31 corrected from "Radix `Dialog`" to "Base-UI `Dialog` via the shadcn wrapper" (`pdlc-ui/src/components/ui/dialog.tsx` wraps `@base-ui/react/dialog`). **CI:** typecheck + lint + Vitest 72/72 (including the new parity test) + Playwright 20/20 all green. Predicts Q re-run clean **READY FOR BUILD** for S3A.2. |
| 2026-04-24 (Fri) | **Felix run** — week of 2026-04-20 | [synthesis/weekly/2026-04-20_friday_signal.md](../06-Resources/Market_intelligence/synthesis/weekly/2026-04-20_friday_signal.md) | 12 signal-log rows; 3 EV candidates queued (Walmart AI, Workvivo Journeys, SA regulatory stack); JEM HR status: **quiet**. intel_feeds.json created + verified; 86 articles ingested (10 feeds). Mining sector: two direct WT use-case validations from live ingest. |
| 2026-04-24 | **Skill pipeline Phase 0** — park `pdlc-ui`, remove Gatekeeper | [plans/skill-pipeline/README.md](../plans/skill-pipeline/README.md); git tag `freeze/skills-pipeline-pivot` | **`pdlc-ui` initiative frozen** in plan docs (FROZEN banners); live methodology is the **skill pipeline** (Felix → Moneypenny → design-prompt → M+Q → Bond → E2E). **Deleted** `.claude/skills/gatekeeper-custom/`; PR discipline → **`babysit`** + frozen R16 tables. **SessionStart** hook loads excerpt of skill-pipeline README + full **`System/icp.md`**. |
| 2026-04-24 | **Skill pipeline Phase 0 — CLOSEOUT** | [`plans/PDLC_UI/seeds/_archived-2026-04-24/`](../plans/PDLC_UI/seeds/_archived-2026-04-24/README.md) | Archived **s3a1, s3a2, s3b, s4–s8** seeds; updated backlinks in `sprint-backlog`, `plan`, `skill-agent-map`, `schema-initiative-v0`, `lifecycle-transitions`, Research strategies, Dex skills, `pdlc-ui/docs/design/*`. **Phase 0 formally complete** — proceed **S2 Moneypenny**. |
