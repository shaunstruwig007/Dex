---
prd_id: login-account-activation
lifecycle: done
...
follow_up_tasks:
  - Activation funnel events → Product Analytics
---

# Login & Account Activation

**Status:** Done (Essential) — agent-oriented retrofit  
**Target:** Frontline workers activating accounts, logging in via OTP, recovering access, landing on Feed  
**Estimated Effort:** Ongoing; OTP and tenant routing hardening
...
**Related PRDs:** [Tenant_Management.md](./Tenant_Management.md) (QR, identifiers, tenant state), [communication_service.md](./communication_service.md) (SMS OTP), [Feed.md](./Feed.md) (typical post-login landing). **Integration map:** [PRD_Product_Map.md](./PRD_Product_Map.md).

---

## The Job to Be Done

Users **discover the right tenant**, **activate** via SMS/email flows, **log in** with OTP/password per policy, and **recover access** — then land on the **Feed** as default surface.

**User value:** Reduces failed activations; foundational for all app features.

---

## Work Packages

### WP-Tenant: Routing & identifiers (P0)
...
### WP-Activate: Invite & first login (P0)
...
### WP-OTP: Login & recovery (P0)

**Priority:** P0  
**Dependencies:** Communication SMS/email  
**Files:** TBD  
**VPS-eligible:** No

### WP-Landing: Post-login (P0)

**Priority:** P0  
**Dependencies:** [Feed.md](./Feed.md)  
**Files:** Client TBD  
**VPS-eligible:** No
...
- OTP delivery and rate limits per BDD.  
- Wrong tenant / identifier edge cases per AC.

---

## Satisfaction Metric

**Overall Success:** BDD Login suite pass **≥ 95%**; support ticket reduction for “didn’t receive invite” aligned with Communication targets.

**Measured by:** QA + support metrics.
...
`analytics_tool: none`. Funnel events listed in [Product_Analytics.md](./Product_Analytics.md).
...
- **SMS-first** activation path for frontline.  
- **Feed** as happy-path landing (Product Map §3).
...
```bash
grep -c "Acceptance criteria (BDD)" "06-Resources/PRDs/Login_Account_Activation.md"
# PASS: >= 1

grep -c "Feed.md" "06-Resources/PRDs/Login_Account_Activation.md"
# PASS: >= 1
```
...
**Scout priorities:** Duplicate identifiers across sub-companies (Problem Statement).
...
- Features not in legacy Login doc without steerco.
...
**Related:** [Tenant_Management.md](./Tenant_Management.md) (QR, codes, identifiers, disabled tenant), [communication_service.md](./communication_service.md) (SMS OTP), [Feed.md](./Feed.md) (post-login landing).  
