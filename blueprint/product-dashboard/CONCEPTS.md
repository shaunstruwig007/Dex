...
- **`build_initiatives_from_prds.py`** (run from `blueprint/product-dashboard/`) regenerates **`initiatives.json`** and **Product_ideas** stubs from PRDs.
- **`start_product_dashboard.sh`** serves the **vault root** so `fetch()` paths like `06-Resources/Product_ideas/...` resolve.

## Location

**`blueprint/`** lives at the **vault root** (sibling of `06-Resources/`).
